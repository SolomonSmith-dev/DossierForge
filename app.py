...
from modules.osint import search_social_media, search_emails, check_breach_data, search_github_info, get_osint_summary

app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'dev-key-change-in-prod')  # For flash messages
...
@app.route('/')
def index():
    dossiers = [d for d in os.listdir(DOSSIERS_DIR) if os.path.isdir(os.path.join(DOSSIERS_DIR, d))]
...
def dossier_overview(name):
    ...
    return render_template('dossier_overview.html', 
                         overview=overview, 
                         name=name, 
...
def run_social_media_search(name):
    ...
    try:
        result = search_social_media(target, target_dir)
...
def run_breach_check(name):
    ...
    try:
        result = check_breach_data(email, target_dir)
...
def run_github_search(name):
    ...
    try:
        result = search_github_info(username, target_dir)
